...
fn scanf32 () -> f32 {
 ...
 let fbuf: f32 = buf.trim().parse().expect("не получилось сделать флоат");
 return fbuf;
}
...
{   
    ...
    t+=shag;
    k1 = xl[i1] - xl[i1].powi(3) - delta*yl[i1] + gamma * (t*w).cos();
    k2 = xl[i1]+shag2 - (xl[i1]+shag2).powi(3) - (delta*yl[i1]+shag2*k1) + gamma * (t*w).cos();
    k3 = xl[i1]+shag2 - (xl[i1]+shag2).powi(3) - (delta*yl[i1]+shag2*k2) + gamma * (t*w).cos();
    k4 = xl[i1]+shag - (xl[i1]+shag).powi(3) - (delta*yl[i1]+shag*k3) + gamma * (t*w).cos();
/* */   y = yl[i1] + (shag * (k1+2.0*k2+2.0*k3+k4)/6.0);
...
{   
    let i1:usize = i as usize;  //костыль, ибо раст не берет интеджер как индекс списка
    let vecm1:Vector2 = Vector2::new(xstart + mashtab * xl[i1-1], ystart + mashtab * yl[i1-1]);
    let vecm2:Vector2 = Vector2::new(xstart + mashtab * xl[i1] , ystart + mashtab * yl[i1]);
